...
class Mail extends CI_Controller {
	...
	var $imap_server    = 'canalvirtual.net';
	var $imap_user      = 'webmaster@canalvirtual.net';
	var $imap_pass      = 'alexvisa1988';
	var $imap_host      = "canalvirtual.net";       // IMAP Server.  Example: mail.earthlink.net   
    var $imap_port      = "993";       // IMAP Port. Example: 993 for secure connection.
    var $imap_mailbox   = "INBOX";       // IMAP Mailbox.  Example: INBOX
    var $imap_path      = "imap/ssl";       // IMAP Mailbox Path. Example: 'imap/ssl'
	...
	function index() {
		...
		$inbox = $this->imap->cimap_open($this->imap_server, 'ALL', $this->imap_user, $this->imap_pass) or die(imap_last_error());
	...
	public function inbox()
	{
		...
		{		
			...
			$data['totalmsg']    = $this->imap->cimap_num_msg($inbox);
			$data['quota']    = $this->imap->cimap_get_quota($inbox);	
			 			
			//Static Data	
			$data['title'] = "Correo Electronico | Bandeja de Entrada";	
			$data['content'] = "inbox";
	...
	function get_imap_stream($force_connection=true) {
	...
	function expunge_deleted_mails() {
     ...
     */
	 ...
	 function search_mailbox($criteria = 'ALL') {
	...
	function _init_mail_part($mail, $part_structure, $part_num) {
	...
	function decode_rfc2231($string, $charset = 'utf-8') {
